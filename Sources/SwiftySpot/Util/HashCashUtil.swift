...
final class HashCashUtil {
    ...
    static func solveClTokenChallengeAsync(prefix: String, length: Int32, completion: @escaping (String?) -> Void) {
        let prefixBytes = StringUtil.hexStringToBytes(prefix)
        let contextDigest = CryptoUtil.sha1(buffer: [])//Empty context for cl token challenge
        ...
        solveHashCashAsync(prefix: prefixBytes, length: length, random: seed) { hashcash in
            ...
            let solvedStr = StringUtil.bytesToHexString(solved)
    ...
    static func solveAuthChallenge(context: Data, prefix: Data, length: Int32) -> [UInt8]? {
        ...
        let contextDigest = CryptoUtil.sha1(buffer: contextBytes)
        let seed = [UInt8].init(contextDigest[12...19])
    ...
    fileprivate static func solveHashCash(prefix: [UInt8], length: Int32, random: [UInt8], timeoutInS: UInt = 10) -> [UInt8]? {
        let bigEndian = true
        guard var l1: Int64 = BitConvertUtil.getVal(random, bigEndian: bigEndian) else {
#if DEBUG
        ...
        while !timeout {
            ...
            let l1Bytes = BitConvertUtil.getBytes(l1, bigEndian: bigEndian)
            let l2Bytes = BitConvertUtil.getBytes(l2, bigEndian: bigEndian)
            buff.append(contentsOf: l1Bytes)
            buff.append(contentsOf: l2Bytes)
            let digest = CryptoUtil.sha1(buffer: buff)
            guard let buffLong: Int64 = BitConvertUtil.getVal(digest, offset: 12, bigEndian: bigEndian) else {
#if DEBUG
    ...
    fileprivate static func solveHashCashAsync(prefix: [UInt8], length: Int32, random: [UInt8], tasks: UInt = 5, taskIterations: UInt = 400000, timeoutInS: UInt = 8, completion: @escaping ([UInt8]?) -> Void) {
        guard let startL1: Int64 = BitConvertUtil.getVal(random, bigEndian: true) else {
#if DEBUG
            print("Parse Int64 fail from random bytes")
#endif
            completion(nil)
    ...
    fileprivate static func hashCashIteration(taskIndex: UInt, prefix: [UInt8], l1: inout Int64, l2: inout Int64, length: Int32) -> [UInt8]? {
        var buff: [UInt8] = [UInt8].init(prefix)
        let l1Bytes = BitConvertUtil.getBytes(l1, bigEndian: true)
        let l2Bytes = BitConvertUtil.getBytes(l2, bigEndian: true)
        buff.append(contentsOf: l1Bytes)
        buff.append(contentsOf: l2Bytes)
        let digest = CryptoUtil.sha1(buffer: buff)
        guard let buffLong: Int64 = BitConvertUtil.getVal(digest, offset: 12, bigEndian: true) else {
#if DEBUG
            print("Parse Int64 fail from SHA1 digest")
#endif
            l1 += 1
